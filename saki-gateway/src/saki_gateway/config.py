...
class DashboardSecurityConfig:
    enabled: bool = True
    password: str = "sha256:240be518fabd2724ddb6f04eeb1da5967448d7e831c08c8fa822809f74c720a9"
...
class AppConfig:
    ...
    dashboard_security: DashboardSecurityConfig = field(default_factory=DashboardSecurityConfig)
...
def _apply_env_overrides(config: AppConfig) -> AppConfig:
    ...
    config.action_api.base_url = os.getenv("SAKI_ACTION_BASE_URL", config.action_api.base_url)
    config.action_api.api_key = os.getenv("SAKI_ACTION_API_KEY", config.action_api.api_key)
    config.action_api.model = os.getenv("SAKI_ACTION_MODEL", config.action_api.model)

    config.search_api.base_url = os.getenv("SAKI_SEARCH_BASE_URL", config.search_api.base_url)
    config.search_api.api_key = os.getenv("SAKI_SEARCH_API_KEY", config.search_api.api_key)
    config.search_api.model = os.getenv("SAKI_SEARCH_MODEL", config.search_api.model)

    config.notion_sync.enabled = _env_flag("SAKI_NOTION_ENABLED", config.notion_sync.enabled)
    config.notion_sync.token = os.getenv("SAKI_NOTION_TOKEN", config.notion_sync.token)
    config.notion_sync.database_id = os.getenv("SAKI_NOTION_DATABASE_ID", config.notion_sync.database_id)
...
class ConfigStore:
    def __init__(self, path: Path):
        ...
        if self.path.exists():
            ...
            if example_path.exists():
                self.path.write_text(example_path.read_text(encoding="utf-8"), encoding="utf-8")
                self.load()
            else:
                self.save()
    ...
    def load(self) -> AppConfig:
        ...
        self._config.dashboard_security.password = normalize_dashboard_password(self._config.dashboard_security.password)
        return self._config
